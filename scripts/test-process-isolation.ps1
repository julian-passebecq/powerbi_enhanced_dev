...
foreach ($app in @('ReportStudio', 'FabricToolbox')) {
    ...
    $pattern = '^(TabularEditor|TOMWrapper|PbiBench\.(App|ModelEditor|Semantic))(\.|$)'
    if ($app -eq 'ReportStudio') { $pattern = '^(TabularEditor|TOMWrapper|Microsoft\.Identity|PbiBench\.(App|ModelEditor|Semantic|Fabric|FabricToolbox))(\.|$)' }
