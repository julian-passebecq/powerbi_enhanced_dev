...
public partial class MainWindow
{
    ...
    private async Task RunAuthoringSmokeAsync(string outputRoot, List<string> checks)
    {
        ...
        await UpdateSessionAsync(); OpenAuthoringTool("Perspectives", true); await PaintAsync(); Capture(outputRoot, "perspectives");
        Check(AuthoringVisuals(metadataEditors!).OfType<DataGrid>().Any(grid => !grid.IsReadOnly && grid.Columns.Count > 0), "Perspective matrix remains editable under the shell's default grid style", checks);
        ...
        await UpdateSessionAsync(); OpenAuthoringTool("Translations", true); await PaintAsync(); Capture(outputRoot, "translations");
        Check(AuthoringVisuals(metadataEditors!).OfType<DataGrid>().Any(grid => !grid.IsReadOnly && grid.Columns.Count > 0), "Translation matrix binds editable draft cells", checks);
        handler.UndoManager.Undo(); handler.UndoManager.Undo();
