        ...
        if (args.Length == 2 && args[0] == "--smoke-test")
        {
            window.Loaded += async (_, _) =>
            {
                try
                {
                    ...
                    if (window.PageCount != 5 || AppDomain.CurrentDomain.GetAssemblies().Any(a => a.GetName().Name is "TOMWrapper" or "TabularEditor" or "PbiBench.ModelEditor")) throw new InvalidOperationException("Toolbox process isolation failed.");
                    File.WriteAllText(args[1], "Toolbox WPF launch: 5 pages, shared Fabric service, no loaded TE2/ModelEditor assembly. No authenticated target used."); app.Shutdown(0);
                }
...
{
    private readonly System.Net.Http.HttpClient http = FabricHttp.CreateClient();
    private readonly IFabricAuthenticator auth = new EntraPublicClientTokenProvider();
    ...
    {
        catalog = new(http, auth); sql = new(auth);
        ...
        Add("Home", Note("Fabric Toolbox · separate .NET 10 process\n\nWorkspace/item inventory and OneLake/SQL source exploration use the shared PbiBench.Fabric services. Model editing, DAX, model refresh/deploy and reviewed semantic import remain in PbiBench Semantic IDE.\n\nThis first Toolbox pass is read-only. Operational/admin expansion is deferred. Sign in explicitly in Settings; tokens stay in this process's memory."));
        var explorer = new DockPanel(); var bar = Bar(Button("Load workspaces", async ct => { workspaces.ItemsSource = await catalog.ListWorkspacesAsync(ct); }), workspaces,
            Button("Load all items", async ct => { resolved = null; schema = null; columns.ItemsSource = null; data.ItemsSource = null; items.ItemsSource = await catalog.ListAllItemsAsync(Workspace().Id, ct); }), Button("Browse source", BrowseAsync), Button("Export selection to Semantic IDE…", ExportSelectionAsync));
        DockPanel.SetDock(bar, Dock.Top); explorer.Children.Add(bar); explorer.Children.Add(items); Add("Workspaces", explorer);
        ...
        Add("Operations", Note("Platform inventory is available in Workspaces, including pipelines and notebooks. Job execution, capacity/tenant administration and remote writes are outside this pass. No operation executes when opening this page."));
        ...
        authBar.Children.Add(Button("Sign out", async ct => { await auth.SignOutAsync(ct); workspaces.ItemsSource = null; items.ItemsSource = null; columns.ItemsSource = null; data.ItemsSource = null; resolved = null; schema = null; })); settings.Children.Add(authBar);
        settings.Children.Add(Note("Ownership: PbiBench.FabricToolbox 0.1.0 (original UI). Transport/auth/SQL: shared PbiBench.Fabric; Microsoft.Identity.Client 4.84.2, Microsoft.Data.SqlClient 6.1.6. Independent Fabric update lane. TE2 is not loaded.\n\nUse a .pbifabric.json handoff in Semantic IDE Apps / Tools → Import Fabric selection. The file carries selection identifiers only and cannot authorize a write.")); Add("Settings / About", settings);
        var cancel = new Button { Content = "Cancel current operation", Margin = new Thickness(4) }; cancel.Click += (_, _) => pending?.Cancel(); settings.Children.Add(cancel);
    ...
    private Button Button(string text, Func<CancellationToken, Task> action)
    {
        ...
        button.Click += async (_, _) => { using var ct = new CancellationTokenSource(); pending = ct; foreach (var b in actions) b.IsEnabled = false; try { await action(ct.Token); status.Text = "Completed · " + text; } catch (OperationCanceledException) { status.Text = "Canceled."; } catch (Exception error) { status.Text = error.Message; } finally { pending = null; foreach (var b in actions) b.IsEnabled = true; } }; return button;
    }
